use mini_browser::html::parser::parse_html;
use mini_browser::css::parser::{parse_css, Stylesheet};
use mini_browser::style::{style_tree, merge_default_styles};
use mini_browser::layout::{build_layout_tree, layout as layout_fn, Dimensions, Rect};
use mini_browser::paint::build_display_list;
use std::fs;

fn main() {
    let html = fs::read_to_string("baidu_real.html").expect("Failed to read baidu_real.html");
    println!("=== HTML size: {} bytes ===", html.len());

    let dom = parse_html(&html);
    
    let mut stylesheet = Stylesheet { rules: Vec::new() };
    if let Ok(css) = fs::read_to_string("baidu.css") {
        let parsed = parse_css(&css);
        println!("CSS rules parsed: {}", parsed.rules.len());
        stylesheet.rules.extend(parsed.rules);
    }
    merge_default_styles(&mut stylesheet);
    println!("Total rules: {}", stylesheet.rules.len());

    let styled = style_tree(&dom, &stylesheet);
    let mut layout_box = build_layout_tree(&styled);
    
    let viewport = Dimensions {
        content: Rect { x: 0.0, y: 0.0, width: 1200.0, height: 800.0 },
        padding: Default::default(),
        border: Default::default(),
        margin: Default::default(),
    };
    layout_fn(&mut layout_box, viewport);
    let display_list = build_display_list(&layout_box);
    println!("Display list: {} commands", display_list.len());

    // Export as SVG (viewport width x height)
    let svg_w = 1200;
    let svg_h = 3000; // taller for baidu content
    let mut svg = String::new();
    svg.push_str("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
    svg.push_str(&format!("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"{}\" height=\"{}\" viewBox=\"0 0 {} {}\">\n", svg_w, svg_h, svg_w, svg_h));
    svg.push_str(&format!("  <rect width=\"{}\" height=\"{}\" fill=\"white\"/>\n", svg_w, svg_h));

    for cmd in &display_list {
        match cmd {
            mini_browser::paint::DisplayCommand::SolidColor(rect, color) => {
                if rect.y > svg_h as f32 || rect.y + rect.height < 0.0 { continue; }
                let r = (color.r * 255.0) as u8;
                let g = (color.g * 255.0) as u8;
                let b = (color.b * 255.0) as u8;
                let line = format!(
                    "  <rect x=\"{:.1}\" y=\"{:.1}\" width=\"{:.1}\" height=\"{:.1}\" fill=\"rgb({},{},{})\"/>\n",
                    rect.x, rect.y, rect.width, rect.height, r, g, b
                );
                svg.push_str(&line);
            }
            mini_browser::paint::DisplayCommand::Text(text, rect, color) => {
                if rect.width < 1.0 || rect.height < 1.0 { continue; }
                if rect.y > svg_h as f32 || rect.y + rect.height < 0.0 { continue; }
                let r = (color.r * 255.0) as u8;
                let g = (color.g * 255.0) as u8;
                let b = (color.b * 255.0) as u8;
                let font_size = rect.height.max(8.0).min(72.0);
                let line = format!(
                    "  <text x=\"{:.1}\" y=\"{:.1}\" font-size=\"{:.1}\" fill=\"rgb({},{},{}))\">{}</text>\n",
                    rect.x, rect.y + font_size * 0.85, font_size, r, g, b,
                    html_escape(text)
                );
                svg.push_str(&line);
            }
            mini_browser::paint::DisplayCommand::Border(rect, width, color) => {
                if rect.y > svg_h as f32 || rect.y + rect.height < 0.0 { continue; }
                let r = (color.r * 255.0) as u8;
                let g = (color.g * 255.0) as u8;
                let b = (color.b * 255.0) as u8;
                let line = format!(
                    "  <rect x=\"{:.1}\" y=\"{:.1}\" width=\"{:.1}\" height=\"{:.1}\" fill=\"none\" stroke=\"rgb({},{},{})\" stroke-width=\"{:.1}\"/>\n",
                    rect.x, rect.y, rect.width, rect.height, r, g, b, width
                );
                svg.push_str(&line);
            }
            mini_browser::paint::DisplayCommand::Image(_, rect, _) => {
                if rect.y > svg_h as f32 || rect.y + rect.height < 0.0 { continue; }
                let line1 = format!(
                    "  <rect x=\"{:.1}\" y=\"{:.1}\" width=\"{:.1}\" height=\"{:.1}\" fill=\"#ddd\" stroke=\"#999\"/>\n",
                    rect.x, rect.y, rect.width, rect.height
                );
                let line2 = format!(
                    "  <text x=\"{:.1}\" y=\"{:.1}\" font-size=\"12\" fill=\"#666\">[img]</text>\n",
                    rect.x + 5.0, rect.y + rect.height / 2.0
                );
                svg.push_str(&line1);
                svg.push_str(&line2);
            }
        }
    }

    svg.push_str("</svg>\n");
    fs::write("baidu_render.svg", svg).expect("Failed to write SVG");
    println!("✅ baidu_render.svg generated ({}x{})", svg_w, svg_h);
}

fn html_escape(s: &str) -> String {
    s.replace('&', "&amp;")
     .replace('<', "&lt;")
     .replace('>', "&gt;")
     .replace('\"', "&quot;")
}
